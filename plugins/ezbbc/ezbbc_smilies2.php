...
// MODIF OPITUX
$text = preg_replace("#(?<=[>\s])".preg_quote($smiley_text, '#')."(?=\W)#m", '<img class="smiley" src="'.pun_htmlspecialchars((function_exists('get_base_url') ? get_base_url(true) : $pun_config['o_base_url']).'/img/smilies/'.$smiley_img).'" width="15" height="15" alt="'.substr($smiley_img, 0, strrpos($smiley_img, '.')).'" />', $text);
// END
endif;
?>
