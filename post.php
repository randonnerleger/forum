...
$tag = new tagManager($subject,$_POST['tag1'],$_POST['tag2'],$fid);
...
<!--********************* Modif RL : insertion message -->
						<textarea name="req_message" rows="20" cols="95" tabindex="<?php echo $cur_index++ ?>"><?php echo isset($_POST['req_message']) ? pun_htmlspecialchars($orig_message) : (isset($quote) ? $quote : ( ( $fid == 63 && file_exists( './include/user/post_sorties_asso_prefilled.txt' ) ) ? file_get_contents( './include/user/post_sorties_asso_prefilled.txt' ) : '' )); ?></textarea>
						<?php
						...
						function displayWindowSize(){
							...
							var inputVal = document.getElementById('post').elements['req_message'];
